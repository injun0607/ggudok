  ...
  const fetchItemListData = async () => {
    try {
      ...
      if(response.data.items.length !== 0){
        ...
        dispatch(pagingItem(items.slice(startIndex, endIndex)));
      } else {
...
useEffect(() => {
  ...
  const handleFilteredItems = useMemo(() => {
    ...
    const pagedItems = updateditems.slice(startIndex, endIndex);
    dispatch(pagingItem(pagedItems));
    console.log('4. filtereditems : ', filtereditems)
    console.log('5. pageditems : ', pageditems)
    console.log('6. updateditems : ', updateditems)
    console.log('6. startIndex : ', startIndex)
    console.log('6. endIndex : ', endIndex)
    return updateditems;
  }, [items, selectedPrice, selectedRating, selectedTag, startIndex, endIndex]);
  ...
  useEffect(() => {
    if (!IsLoading) {
      if (selectedPrice || selectedRating || selectedTag.length > 0) {
          ...
          setIsResult(true);
          const pagedFilteredItems = filtereditems.slice(startIndex, endIndex);
          dispatch(pagingItem(pagedFilteredItems));
          console.log('2. IsResult : ', IsResult)
          console.log('2. pageditems : ', pageditems)
        } else if((filtereditems.length === 0)) {
          setIsResult(false);
          dispatch(pagingItem([]));
          console.log('3. IsResult : ', IsResult)
        }
      } else {
        dispatch(pagingItem(items.slice(startIndex, endIndex)))
      }
      ...
      <div className='webwidth'>
        ...
        <div className={style.categorysection}>
          ...
          <section className={style.right}>
            ...
            <Paging page={page} setPage={setPage} count={selectedPrice || selectedRating || selectedTag.length > 0 ? filtereditems.length : items.length} />
          </section>
        </div>
      </div>
    </section>
