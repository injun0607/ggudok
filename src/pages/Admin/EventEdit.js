...
//redux import
import { editEvent } from '../../redux/actions/admin/adminEventsActions';
...
const EventEdit = () => {
  ...
  const [isEventInfoTagval, setEventInfoTagval] = useState(true);
  const [errorMessageInfoTag, setErrorMessageInfoTag] = useState('');
  const [isEventInfoval, setEventInfoval] = useState(true);
  const [errorMessageInfo, setErrorMessageInfo] = useState('');
  const [isEventDateStartval, setEventDateStartval] = useState(true);
  const [errorMessageDateStart, setErrorMessageDateStart] = useState('');
  const [isEventDateEndval, setEventDateEndval] = useState(true);
  ...
  const fetchEventsData = async () => {
    try {
      const response = await axios.get(`/admin/event/${eventId}`);
      const responseReg = await axios.get(`/admin/event/register`);
      ...
      if(data !== 0){
        ...
        setEventSubsId(data.subsId);
        setEventInfoTag(data.infoTag);
        setEventInfo(data.info);
        setEventImage(data.eventImage);
        setEventDateStart(data.startDate);
        setEventDateEnd(data.endDate);
        setIsValid(data.valid);
        dispatch(fetchItemsSuccess(dataSubs));
        dispatch(fetchCategorySuccess(dataCategory));
        dispatch(setSelectedCategory(data.categoryName))
      } else {
        setEventSubsName([]);
        setEventSubsId([]);
        setEventInfoTag([]);
