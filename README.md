...
네이버와 카카오톡 **소셜 로그인 및 회원가입**이 가능합니다.

<table>
  <tr>
    <td><img src="./readme_image/login_naver.png" alt="네이버로그인" /></td>
    <td><img src="./readme_image/login_naver02.png" alt="네이버로그인 추가정보" /></td>
  </tr>
</table>

네이버 `소셜로그인/회원가입`과 회원가입 후 `추가정보입력 페이지`입니다. *(네이버 로그인은 api에서 네이버 로그인 검수요청중이므로 작동되지 않으므로 사진으로 대체합니다.)*

유저가 이메일 회원가입을 선택한 경우, **이메일로 회원가입**할 수 있습니다.
